...
// Fast-timing save (no settle) via indexed-db + string/json only (no configurator).
describe("minimal fast save isolation", () => {
  it("saving tracks fast after facets keeps both", async () => {
    const result = await testWeb(async () => {
      ...
      t.setAttribute("output-selector", "#fast-idb");
    ...
    console.log("MIN_DIAG", JSON.stringify(result));
