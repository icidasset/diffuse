...
/**
 * @typedef {{ name: string; detail: string; isInput: boolean; isOutput: boolean; onRemove: () => void }} ConnectItem
 */
 ...
 */
export function setup({ title, description, formFields, onSubmit }) {
      ...
      <wa-card>
        ...
        <div class="card-body">
          ...
          <div class="button-row">
            <wa-button id="connect-add-input-btn" variant="brand" appearance="filled">
  ...
  return {
    ...
    setItems(items) {
      ...
      litRender(
        html`${items.map(
          ({ name, detail, isInput, isOutput, onRemove }) => html`
