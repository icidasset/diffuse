...
content :
    ...
    -> InfiniteListDeps
    -> Node
content resultant ( sortBy, sortDirection, playlist, favouritesOnly, isProcessing, sources ) deps =
    el
        Zed
        ...
        (if List.isEmpty resultant then
            ...
            case Maybe.map .autoGenerated playlist of
                Just False ->
                    playlistTable resultant deps

                _ ->
                    defaultTable resultant sortBy sortDirection deps
        )
...
playlistTable : List IdentifiedTrack -> InfiniteListDeps -> Node
playlistTable tracks ( infiniteList, isTouchDevice, screenHeight ) =
    table
        [ playlistTableHeader
        , html
            (InfiniteList.view
                (infiniteListConfig isTouchDevice screenHeight playlisTableItem)
                (infiniteList)
                (tracks)
            )
        ]
...
playlisTableItem : Int -> Int -> IdentifiedTrack -> Html TopLevel.Msg
playlisTableItem _ _ ( identifiers, track ) =
    let
        nr =
