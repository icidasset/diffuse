    ...
    case reply of
        ...
        ToggleRememberProgress ->
            model
                |> Lens.modify
                    Audio.lens
                    (\a -> { a | rememberProgress = not a.rememberProgress })
                |> translate
                    SaveSettings
                    ...
                    maybeQueueItem
                        ...
                        |> Maybe.map
                            (Queue.makeEngineItem
                                ...
                                model.sources.collection
                                model.tracks.cached
                                (if model.audio.rememberProgress then
                                    model.audio.progress
...
gatherSettings { audio, backdrop, lastFm, processAutomatically, tracks } =
    ...
    , rememberProgress = audio.rememberProgress
    }
