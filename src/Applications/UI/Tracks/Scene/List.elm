    ...
    Html.div
        ...
        , brick
            ...
            (if not showArtist && not showAlbum then
                ...
                , otherColumn "43%" False (Maybe.withDefault "⚡︎" track.tags.album)
                ]

             else if showArtist && not showAlbum then
                [ favouriteColumn "5.75%" favouritesOnly favIdentifiers derivedColors
                , otherColumn "51.25%" False track.tags.title
                , otherColumn "43%" False (Maybe.withDefault "⚡︎" track.tags.artist)
                ]

             else
                [ favouriteColumn defFavColWidth favouritesOnly favIdentifiers derivedColors
                , otherColumn "37.5%" False track.tags.title
                , otherColumn "29.0%" False (Maybe.withDefault "⚡︎" track.tags.artist)
                , otherColumn "29.0%" True (Maybe.withDefault "⚡︎" track.tags.album)
                ]
            )
        ]
...
playlistItemView favouritesOnly nowPlaying _ selectedTrackIndexes dnd showAlbum darkMode derivedColors _ idx identifiedTrack =
    ...
    brick
        ...
        (if showAlbum then
            ...
            , otherColumn "27.5%" False (Maybe.withDefault "⚡︎" track.tags.artist)
            , otherColumn "27.5%" True (Maybe.withDefault "⚡︎" track.tags.album)
            ]
            ...
            , otherColumn "41.25%" False (Maybe.withDefault "⚡︎" track.tags.artist)
            ]
        )
