...
shuffled timestamp unfilteredTracks s =
    ...
    if howMany > 0 then
        timestamp
            |> Time.posixToMillis
            |> Random.initialSeed
            |> generateIndexes generator howMany Set.empty
            |> Set.toList
            |> List.foldl
                (\idx acc ->
...
-}
generateIndexes : Generator Int -> Int -> Set Int -> Seed -> Set Int
generateIndexes generator howMany squirrel seed =
    let
        ( index, newSeed ) =
            Random.step generator seed
    in
    if Set.member index squirrel then
        generateIndexes generator howMany squirrel newSeed

    else if howMany - 1 > 0 then
        generateIndexes generator (howMany - 1) (Set.insert index squirrel) newSeed

    else
        Set.insert index squirrel
