...
generateIndexes generator howMany squirrel seed =
    let
        ...
        newSquirrel =
            -- We could check if the generated index is already one we have,
            -- but I don't think that ever happens (because of the use of `Random.step`)
            index :: squirrel
    in
    if howMany - 1 > 0 then
        generateIndexes generator (howMany - 1) newSquirrel newSeed
